...
function crawl(root, regionLookup, geojson, o, callback) {
  ...
  async.eachSeries(
    ...
    function(feature, next) {
      readRefs(path.join(root, feature.properties.hobbes.repo.path), feature, 'properties', options, () => {
        feature.properties.hobbes.repo.files = options.files;
        delete options.path;
  ...
  for( var i = 0; i < nodes.length; i++ ) {
    if( nodes[i].properties.terminus === node.properties[uid] ) {
      origins.push({
        ...
        link : nodes[i].properties.hobbes.id
      });
