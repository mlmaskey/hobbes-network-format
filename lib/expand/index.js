...
function expand(opts, callback) {
  if( !opts.properties ) {
    opts.properties = getPaths(opts.node);
  }
  ...
  async.eachSeries(
    ...
    (objectPath, next) => {
      ...
      var filePath = path.join(opts.repoDir, opts.node.properties.hobbes.repo.path, refInfo.parent[refInfo.attr].$ref);
...
function getRefProp(path, object) {
  ...
  for( var i = 0; i < path.length-2; i++ ) {
    if( !tmp[path[i]] ) {
      throw new Error(`Invalid path '${path.join('.')}' for object:\n${JSON.stringify(object, '', '')}`);
    }
  ...
  if( !tmp.$ref ) {
    if( !tmp[path[i]] ) {
      throw new Error(`Path has no $ref '${path.join('.')}' in object:\n${JSON.stringify(object, '', '')}`);
    }
  }
