...
function onCrawlComplete(items, result, getId, callback) {
  ...
  var regions = result.regions.features;
  var includeRegionNodes = [];

  regions.forEach(function(region){
    var last = region.properties.hobbes.region.split('/').pop();

    if( items[region.properties.hobbes.id] || items[last] ) {
      region.properties.hobbes.nodes.forEach(node => includeRegionNodes.push(node));
      region.properties.hobbes.links.forEach(link => includeRegionNodes.push(link));

      if( items[region.properties.hobbes.region] ) {
        delete items[region.properties.hobbes.region];
      } else {
        delete items[last];
      }
    }
  ...
  for( var i = 0; i < includeRegionNodes.length; i++ ) {
    for( var j = 0; j < all.length; j++ ) {
      if( includeRegionNodes[i] === all[j].properties.hobbes.id ) {
        items[all[j].properties.prmname] = true;
        break;
      }
    }
  }
